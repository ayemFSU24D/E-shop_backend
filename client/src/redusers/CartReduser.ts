  ...
  export const CartReducer = (state: CartState, action: ICartAction) => {
    ...
    switch (type) {
      case CartACtionType.ADD_ITEM: {
        const itemExists = state.cart.find((item) => item.product._id === payload.product._id);
      ...
      case CartACtionType.REMOVE_ITEM: {
        const updatedCart = state.cart.filter((item) => item.product._id !== payload.product._id);
